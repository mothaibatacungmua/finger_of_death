...
BertConfig BertConfig::fromMap(map<string, unique_ptr<MapFieldInterface>> mapObject){
    BertConfig ret;
    ...
    ret.hiddenAct = GET_V(mapObject, string, "hiddenAct");
...
map<string, unique_ptr<MapFieldInterface>> BertConfig::toMap(){
    map<string, unique_ptr<MapFieldInterface>> ret;
    ...
    ret["hiddenAct"] = MAKE_V(string, this->hiddenAct);
